...
class DynStartApp(ActionBase):
    ...
    def _insert_extras(self, argument, data):
        return argument % data

    def _execute(self, data=None):
        if self._cwd:
            cwd = self._insert_extras(self._cwd, data)
        else:
            cwd = None

        args = [self._insert_extras(a,data) for a in self._args]
        self._log.debug("Starting app: %r" % (args,))
        try:
            Popen(args, cwd=cwd)
        except Exception, e:
